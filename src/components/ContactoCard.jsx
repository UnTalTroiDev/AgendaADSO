 ...
 * - Botón editar que llama a onEditar(contacto); botón eliminar que llama a onEliminar(id).
 */

import { app } from '../config'

function ContactoCard({ contacto, onEditar, onEliminar }) {
  return (
    <article className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2 self-start sm:self-center shrink-0">
        <button
          ...
          className="px-4 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 text-sm font-medium hover:bg-gray-50 focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition"
        >
          {app.editButtonLabel}
        </button>
        <button
          ...
          className="px-4 py-2 rounded-lg bg-red-500 text-white text-sm font-medium hover:bg-red-600 focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition"
        >
